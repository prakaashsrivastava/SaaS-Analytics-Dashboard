...
const CustomTooltip = ({ active, payload }: RetentionTooltipProps) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-surface border border-border rounded-[8px] p-4 shadow-card animate-in zoom-in-95 duration-200">
...
export function RetentionChart({ data, loading }: RetentionChartProps) {
  ...
  return (
    <Card className="bg-surface border-border rounded-3xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-500 group">
      ...
      <CardContent className="pt-6 px-6">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {data.map((item, index) => (
            ...
            >
              <span className="block text-[8px] font-black text-text-muted uppercase tracking-widest mb-1">
                {item.name}
              </span>
              <span
